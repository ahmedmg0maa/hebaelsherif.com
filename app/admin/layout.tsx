...
import { Calendar, Home, Loader2, LogOut, Menu, MessageSquare, ShoppingCart, User, X } from "lucide-react"
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (pathname !== "/admin/login" && authenticated === false) {
  ...
  if (authenticated === null) {
    return (
      <div className="grid min-h-screen place-items-center bg-background" dir="rtl">
        <div className="flex items-center gap-2 rounded-2xl border border-border bg-card px-5 py-4 text-sm font-bold text-muted-foreground">
          ...
          <div className="border-t border-border p-4">
            <div className="mb-3 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">إدارة الحجوزات والطلبات</p>
              </div>
            </div>
            <Button
              variant="ghost"
